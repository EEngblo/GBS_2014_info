...
private:
	...
	void play_ball(){
		...
		if (debug) cout << "정답 : " << correct_answer << endl; // 디버깅 모드일 때에는 답을 표시함
		...
		cout << cnt << "번 만에 맞추셨습니다!" << endl << endl << "재경기는 r, 종료는 x를 눌러주세요" << endl;
	...
	void print_state(){
		...
		char bb[] = "Balls";
		char bbb[10], sss[10];
		if (this->strike <= 1) strcpy(sss, s); // 스트라이크 수가 0,1개면 단수형
		else strcpy(sss, ss); // 스트라이크 수가 2개 이상이면 복수형

		if (this->ball <= 1) strcpy(bbb, b); // 볼 수가 0,1개면 단수형
		else strcpy(bbb, bb); // 볼 수가 2개 이상이면 복수형
...
int main(){
	...
	Baseball.start(); // Baseball 클래스의 start 함수 호출
	return 0;
}
